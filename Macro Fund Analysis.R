...
######################################################
# Goal: Identify which Vanguard funds best explain MacroFund_excess
# We create a new merged data frame specifically for this regression.

df_lasso_vanguard <- vanguard
df_lasso_vanguard$Fund_Excess_Return <- df_merged$Fund_Excess_Return

vanguard_cols <- unique(grep("^V[A-Z]{4}$", names(df_lasso_vanguard), value = TRUE))

for (col in vanguard_cols) {
  excess_col <- paste0(col, "_excess")
  df_lasso_vanguard[[excess_col]] <- df_lasso_vanguard[[col]] - rf_data$Rf 
}

# X matrix and y vector
vanguard_excess_cols <- paste0(vanguard_cols, "_excess")
X_vanguard <- as.matrix(df_lasso_vanguard[, vanguard_excess_cols])
y_lasso_vanguard <- df_lasso_vanguard[["Fund_Excess_Return"]]
...
X_selected <- df_lasso_vanguard[complete_rows, best_predictors_vanguard]

lm_vanguard <- lm(y_lasso_vanguard ~ ., data = as.data.frame(X_selected))
...
############################################################
# SECTION 8: LASSO ON ALL MARKET PREMIA FACTORS
############################################################
...
############################################################
# SECTION 13: 2008-2009 CRISIS DATA & EXPECTED RETURN
############################################################

crisis_factors <- read_csv("factor_data_2008_2009.csv") %>%
